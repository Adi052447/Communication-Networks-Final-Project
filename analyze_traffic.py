...
pcap_dir = Path("../res")  # לדוגמה, קובצי ה-PCAP נמצאים בתיקייה "pcaps"

# הגדרת קובצי PCAP של 5 אפליקציות עם נתיב יחסי
pcap_files = {
     "Chrome": pcap_dir / "chrome_record.pcapng",
     "FireFox": pcap_dir / "firefox_record.pcapng",
     "Spotify": pcap_dir / "spotify_record.pcapng",
     "YouTube": pcap_dir / "youtube_record.pcapng",
    "Zoom": pcap_dir / "zoom_record.pcapng"
}
...
#
# יצירת מקרא המבוסס על האפליקציות שבנתונים
legend_patches = [Patch(color=color, label=app) for app, color in app_colors.items() if app in df_packets["Application"].unique()]
plt.legend(handles=legend_patches, title="Application")

plt.show()

# יצירת גרף התפלגות גדלי חבילות עם מקרא ידני
sns.set_theme(style="darkgrid")
plt.figure(figsize=(14, 10))
ax = sns.histplot(
    data=df_packets,
...
if labels:
    plt.legend(handles=handles, labels=labels, title="Application")
else:
    print(" Warning: No labels found for legend!")
...
ax = sns.histplot(
    ...
    multiple="stack"  # או "stack"/"dodge" לפי העדפה
)
...
# sorted_packets = {}  # מילון לאחסון התוצאות
#
# # בדיקת מספר החבילות לכל אפליקציה
# for app in pcap_files.keys():
#     num_packets = len(all_packets.get(app, []))
#     print(f" בדיקת מספר חבילות באפליקציה {app}: {num_packets}")
#     if num_packets == 0:
#         print(f"️ אין חבילות עבור {app}, מדלג...")
#         continue
#
#     sorted_packets[app] = sorted(all_packets[app], key=lambda p: float(p.sniff_timestamp))
#
...
#
#     print(f" {app}: מספר bins תקפים = {len(valid_bins)}")
#
#     # 4. חישוב ה-"Large Gap":
...
#
#         if gap > 0:
#             print(f"{app} - Gap שנמצא: {gap:.4f} שניות")
...
# print(df_large_gaps["Application"].value_counts())
#
# sns.set_theme(style="darkgrid")
# plt.figure(figsize=(10, 6))
# ax = sns.histplot(
